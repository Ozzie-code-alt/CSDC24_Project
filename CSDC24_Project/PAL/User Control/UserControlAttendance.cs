...
namespace CSDC24_Project.PAL.User_Control
{
    public partial class UserControlAttendance : UserControl
    {
        ...
        {
            InitializeComponent();
            dataGridViewMrkAttendance.Columns["Column1"].Visible = false;
            dataGridViewMrkAttendance.Columns["Column5"].Visible = false;
        }
        ...
        private void tabPageMarkAttendance_Leave(object sender, EventArgs e)
        {
            if (comboBoxClass.SelectedIndex != -1)
            {
                ...
                if (Attendance.Attendance.IsMarkAttendance(dateTimePickerDate.Text, comboBoxClass.SelectedItem.ToString(), sql))
                {
                    foreach (DataGridViewRow row in dataGridViewMrkAttendance.Rows)
                    {
        ...
        private void dataGridViewMrkAttendance_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (comboBoxClass.SelectedIndex != -1)
            {
